...
let data;
let subcategoriesQuery = `SELECT * FROM "products"."subcategories"`;

exports.handler = async (event, context) => {
  console.log('Received event:', JSON.stringify(event, null, 2));
  console.log('remaining time =', context.getRemainingTimeInMillis());
  let AthenaParams = {
    QueryString: subcategoriesQuery,
    ResultConfiguration: {
  ...
  try {
    ...
    const params = {
      ...
      TopicArn: `arn:aws:sns:us-east-1:${process.env.AWS_ACCOUNT_ID}:selectSubcategoriesQueryId`,
      Subject: "Select Subcategories Query ID"
    };
